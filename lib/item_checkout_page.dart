...
class _ItemCheckoutPageState extends State<ItemCheckoutPage> {
  ...
  void initState() {
    ...
    cartMap.forEach((key, value) {
      keyList.add(int.parse(key)); // 정수타입으로 변환
    });

    // !fireStore에서 데이터 가져오는 Ref 변수
    if (keyList.isNotEmpty) {
      productListRef = database
          ...
          .where("productNo", whereIn: keyList); // productNo가 keylist에 있는 것들만
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            if (cartMap.isNotEmpty) // 장바구니 리스트가 있는 경우만
              StreamBuilder(
                stream: productList,
                        ...
                        if (formKey.currentState!.validate()) {
                          if (selectedPaymentMethod == "결제수단선택") {
                            showDialog(
                              context: context,
                              barrierDismissible:
                                  true, // 다이얼로그창 밖의 영역을 눌러 벗어나는 것을 방지
                              builder: (context) {
                                return BasicDialog(
  ...
  Widget inputTextField({
    ...
    int? currentMaxLength, // null 허용
    bool isObscure = false, // 불분명
    ...
    return Padding(
      ...
      child: TextFormField(
        validator: (value) {
          ...
          return null; // null을 return하지 않으면 다음으로 넘어가지 않음.
        },
  ...
  Widget paymentMethidDropdownButton() {
    return Container(
      width: double.infinity, // 가득 채움
